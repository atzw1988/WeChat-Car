...
Page({
  ...
  lockList: function (){
    ...
    app.func.req('/park/selectpark', { parking_weiyi_no: lockDetailList, pageIndex: searchPageNum }, function (res) {
    // wx.request({
    //   url: http.reqUrl + '/park/selectpark',
    //   data: { parking_weiyi_no: lockDetailList, pageIndex: searchPageNum },
    //   method: 'POST',
    //   header: {
    ...
    //   success: function (res) {
        console.log(res);
        that.setData({
          oAddress: res.data.map[0].address,
          oName: res.data.map[0].parking_name,
          oLat: res.data.map[0].latitude,
          oLng: res.data.map[0].longitude
        ...
        if (res.data.map.length > 0) {
          ...
          if (res.data.tp == 1 || res.data.tp == 0) {
            that.setData({
